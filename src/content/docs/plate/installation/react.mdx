...
<Callout type="warning" title="Prerequisites">
  ...
  [shadcn/ui](https://ui.shadcn.com/docs/installation) (adapted for your framework, e.g.,
  Vite) and [Plate UI](/docs/installation/plate-ui).
</Callout>
...
<Callout type="info">
  ...
  re-renders. For a non-memoized version, use `createPlateEditor`.
</Callout>
...
<Callout type="info">
  The `basic-nodes-kit` includes all the basic plugins (bold, italic, underline, headings,
  blockquotes, etc.) and their components that we'll use in the following steps.
</Callout>
