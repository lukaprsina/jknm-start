...
<Callout type="info">
  `createSlateEditor` creates a raw Plate editor instance suitable for server-side logic,
  data transformation, or preparing content for static rendering. It provides the same API
  as the React version but without browser dependencies.
</Callout>
...
async function processDocument(value: any[]) {
  ...
  editor.tf.setNodes({ type: "h2" }, { at: [], match: (n) => n.type === "h1" });
