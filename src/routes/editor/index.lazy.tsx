...
export const get_markdown = createServerFn().validator(get_markdown_validator).handler(async ({ data }) => {
    ...
    <>
      <Card className="my-4 p-4 flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <label htmlFor="article-id" className="text-sm font-medium text-muted-foreground">Article ID</label>
          <Input
            id="article-id"
            type="number"
            min={1}
            ...
            onChange={e => setArticleId(e.target.value ? Number(e.target.value) : undefined)}
        ...
        <div className="flex gap-2 flex-wrap">
          <Button onClick={async () => {
            ...
            const markdown = await get_markdown({ data: { article_id: articleId } })

            if (!markdown) {
              console.warn("No markdown content found for the article ID:", articleId);
              return;
            }

            console.log("Loaded markdown content:", { articleId, markdown });
            const new_value = editor.getApi(MarkdownPlugin).markdown.deserialize(markdown);
