      ...
      <InlineCombobox element={element} trigger="#">
        ...
        <InlineComboboxContent>
          ...
          {tags.map((tag) => (
            ...
            >
              <span className={`h-3 w-3 rounded-full bg-${tag.color}-500 mr-2`} />#
              {tag.name}
            </InlineComboboxItem>
          ))}
        </InlineComboboxContent>
...
export function TagElement({ element, ...props }) {
  ...
  return (
    <PlateElement
      {...props}
      className={cn(
        "bg-primary/10 text-primary inline-block rounded-md px-1.5 py-0.5 align-baseline text-sm font-medium",
...
<API name="TriggerComboboxPluginOptions">
  <APIOptions>
    ...
    <APIItem name="trigger" type="RegExp | string[] | string">
      Character(s) that trigger the combobox. Can be: - A single character (e.g. '@') - An
      array of characters - A regular expression
    </APIItem>
    <APIItem name="triggerPreviousCharPattern" type="RegExp" optional>
      Pattern to match the character before trigger. - **Example:** `/^\s?$/` matches
