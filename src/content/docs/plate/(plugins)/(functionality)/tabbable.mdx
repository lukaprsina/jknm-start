...
<API name="TabbablePlugin">
  <APIOptions>
    <APIItem name="query" type="(event: KeyboardEvent) => boolean" optional>
      Enable/disable plugin dynamically. - **Default:** `() => true`
    </APIItem>
    <APIItem name="globalEventListener" type="boolean" optional>
      Add event listener to document instead of editor. - **Default:** `false`
    </APIItem>
    <APIItem
      name="insertTabbableEntries"
      type="(event: KeyboardEvent) => TabbableEntry[]"
      optional
    >
      Add additional tabbable entries outside editor. - **Default:** `() => []`
    </APIItem>
    <APIItem name="isTabbable" type="(tabbableEntry: TabbableEntry) => boolean" optional>
      Determine if element should be tabbable. - **Default:** `(tabbableEntry) =>
      editor.api.isVoid(tabbableEntry.slateNode)`
    </APIItem>
  </APIOptions>
</API>
