...
<Callout type="warning" title="Performance Consideration">
  Using `editor.tf.setValue` will re-render all nodes on each call, so it should be used
  carefully and sparingly. It may impact performance if used frequently or with large
  documents.
</Callout>
