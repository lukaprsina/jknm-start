...
<API name="TrailingBlockPlugin">
  <APIOptions>
    ...
    <APIItem name="type" type="string" optional>
      Type of the trailing block to insert. - **Default:** `'p'` (paragraph)
    </APIItem>
    <APIItem name="allow" type="string[]" optional>
      Filter nodes matching these types. Only these types will be considered valid
      trailing blocks. - **Default:** `[]` (all types allowed)
    </APIItem>
    <APIItem name="exclude" type="string[]" optional>
      Filter nodes not matching these types. These types will not trigger trailing block
      insertion. - **Default:** `[]` (no types excluded)
    </APIItem>
    <APIItem name="filter" type="(node: TNode) => boolean" optional>
      Custom filter function to determine if a node should trigger trailing block
      insertion.
    </APIItem>
  </APIOptions>
</API>
